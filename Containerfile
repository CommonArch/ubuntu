...
RUN if [ "$VARIANT" = nvidia ]; then apt-get update; DEBIAN_FRONTEND=noninteractive apt-get install -yq linux-modules-nvidia-580-generic nvidia-driver-580; fi
...
RUN wget -O /usr/bin/system https://github.com/CommonArch/system-cli/raw/refs/heads/$CORE_BRANCH/usr/bin/system; chmod 755 /usr/bin/system; \
    mkdir -p /usr/lib/dracut/modules.d/77commonarch; \
    wget -O /usr/lib/dracut/modules.d/77commonarch/handle-update.sh https://github.com/CommonArch/system-cli/raw/refs/heads/$CORE_BRANCH/usr/lib/dracut/modules.d/10commonarch/handle-update.sh; chmod 755 /usr/lib/dracut/modules.d/77commonarch/handle-update.sh; \
    wget -O /usr/lib/dracut/modules.d/77commonarch/module-setup.sh https://github.com/CommonArch/system-cli/raw/refs/heads/$CORE_BRANCH/usr/lib/dracut/modules.d/10commonarch/module-setup.sh; chmod 755 /usr/lib/dracut/modules.d/77commonarch/module-setup.sh
